...
Every date below comes from the commit that made the change. Those commits are
in [anicat-tauri-archive](https://github.com/bonkedbythonk/anicat-tauri-archive),
not here: this repository starts at the native app's own first commit. The
archive has three grafted root commits, so `git log` from any single branch
