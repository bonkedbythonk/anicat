...
Anime/manga viewers with an AniList account who want to watch, read, and track without a browser. Public GitHub repo, restarted at 1.0.0 in September 2026 with the native app alone; everything before it, up to v6.0.1, is in anicat-tauri-archive. The audience today is the developer on a MacBook; the iPhone build runs but is held back from the release, since installing it needs a sideloading tool on a computer and lasts a week on a free Apple ID. First-run clarity still matters because the repo is public.
