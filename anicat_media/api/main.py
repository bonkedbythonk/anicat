...
def _install_signal_handlers() -> None:
    """Install signal handlers that kill children before exiting.

    When the Rust shell sends SIGTERM to the process group, this handler
    ensures all descendant processes (uvicorn workers, MPV, downloads)
    are terminated so nothing leaks across sessions.
    """

    def _cleanup_and_exit(signum: int, _frame) -> None:
        sig_name = signal.Signals(signum).name
        logger.info("Received %s — cleaning up child processes", sig_name)
        pid = os.getpid()
        try:
            # Kill all children of this process so MPV/uvicorn don't survive.
            ...
            logger.debug("killpg failed (children may already be gone): %s", e)
        os._exit(0)
