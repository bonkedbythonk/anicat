...
class GeneralConfig(BaseConfig):
    ...
    pygment_style: Literal[
        ...
        default=defaults.GENERAL_PYGMENT_STYLE,
        description=desc.GENERAL_PYGMENT_STYLE,  # type: ignore
    )
    ...
    def sanitize_provider(cls, v):
        ...
        if isinstance(v, str):
            if v not in valid_values:
                return ProviderName.ANIZONE
        elif isinstance(v, ProviderName):
            if v.value not in valid_values:
                return ProviderName.ANIZONE
        return v

    @field_validator("provider_fallbacks", mode="before")
    @classmethod
    def sanitize_provider_fallbacks(cls, v):
        ...
        if isinstance(v, list):
            ...
            for item in v:
                ...
                elif isinstance(item, ProviderName):
                    if item.value in valid_values:
                        sanitized.append(item)
            return sanitized
        return v

    @field_validator("manga_provider", mode="before")
    @classmethod
    def sanitize_manga_provider(cls, v):
        ...
        if isinstance(v, str):
            if v not in valid_values:
                return MangaProviderName.MANGAKATANA
        elif isinstance(v, MangaProviderName):
            if v.value not in valid_values:
                return MangaProviderName.MANGAKATANA
        return v
