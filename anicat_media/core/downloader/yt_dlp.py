...
from yt_dlp.utils import sanitize_filename  # type: ignore[reportMissingModuleSource]
...
class YtDLPDownloader(BaseDownloader):
    def download(self, params: DownloadParams) -> DownloadResult:
        ...
        try:
            ...
            if TORRENT_REGEX.match(params.url):
                raise AnicatError(
                    "Torrent downloads are not supported in this build. "
                    "The torrent module is not included."
                )
            else:
    ...
    def _merge_subtitles(
        ...
        if not self.FFMPEG_EXECUTABLE:
            raise AnicatError("Please install ffmpeg in order to merge subs")
