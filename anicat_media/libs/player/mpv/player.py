...
class MpvPlayer(BasePlayer):
    ...
    def __init__(self, config: MpvConfig, player_type: str = "external"):
        ...
        # Clean up any orphaned MPV processes from prior sessions that may
        # still hold VideoToolbox hardware decoder instances, causing black
        # screens across all video players on the system.
        if sys.platform == "darwin":
            self._kill_orphaned_mpv_processes()

    @staticmethod
    def _kill_orphaned_mpv_processes() -> None:
        """Terminate stale MPV processes that were not properly cleaned up.

        Orphaned MPV instances hold VideoToolbox hardware decoder sessions
        on macOS.  When the session pool is exhausted (typically 16-32 slots
        system-wide) every video-playing app — including Safari/Chrome for
        Netflix — shows a black screen instead of video.

        We use ``pgrep`` / ``pkill`` because they are available on every
        macOS install and avoid parsing ``ps`` output portably.
        """
        try:
            ...
            for line in result.stdout.strip().splitlines():
                ...
                # Skip Anicat's own worker / script that may embed "mpv"
                # in its own command line.
                if "anicat" in cmdline.lower() and "mpv" not in cmdline.lower().split("/")[-1]:
                    continue

                logger.info(
                    "Terminating orphaned MPV process pid=%d cmdline=%s", pid, cmdline
                )
                try:
                    os.kill(pid, signal.SIGTERM)
                except (ProcessLookupError, PermissionError) as e:
                    logger.debug("Could not terminate pid=%d: %s", pid, e)
        except Exception as e:
            logger.debug("Orphaned MPV cleanup skipped: %s", e)
        ...
        if resources_dir:
            ...
            if shader_profile != "off":
                ...
                if os.path.exists(bundled_shaders_dir):
                    # Use S (Small/Fast) shaders for optimal low GPU thermal impact
                    size_suffix = "S"
                    ...
                    restore_path = os.path.join(
                        bundled_shaders_dir, f"Anime4K_Restore_CNN_{size_suffix}.glsl"
                    )
                    upscale_path = os.path.join(
                        bundled_shaders_dir, f"Anime4K_Upscale_CNN_x2_{size_suffix}.glsl"
                    )
                    ...
                    if shaders_to_load:
                        ...
                        logger.info(f"Using Anime4K Mode A (CNN_{size_suffix}) upscaling shaders.")
            else:
                logger.info(
