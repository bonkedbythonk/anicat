...
def test_anime_provider(AnimeProvider: Type[BaseAnimeProvider]):
    ...
    for i, stream in enumerate(episode_streams):
        print(f"\n[{i + 1}] Server: {stream.name}")
        
        # Print subtitles if any exist
        if stream.subtitles:
            print("    Subtitles:")
            for sub in stream.subtitles:
                print(f"      - {sub.language or 'Unknown'}: {sub.url}")
        else:
            print("    Subtitles: None (Likely Hard Sub or Dub)")
            
        # Print stream links
        print("    Links:")
        for idx, link in enumerate(stream.links):
            hls_status = "HLS" if link.hls else "Direct/HTTP"
            trans_val = link.translation_type.value if hasattr(link.translation_type, "value") else str(link.translation_type)
            print(f"      - Link {idx + 1}: {link.link} ({hls_status}, translation: {trans_val}, quality: {link.quality})")
        
        if stream.headers:
            print(f"    Headers: {stream.headers}")
            
    print("")
    stream = episode_streams[int(input(f"Select your preferred server (1-{len(episode_streams)}): ")) - 1]
