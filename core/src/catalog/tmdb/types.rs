...
//!
//! - `id` is the raw TMDB id. The Tauri build shifted it into a numeric band
//!   so it could not be mistaken for an AniList id in a schema that had only
//!   one id column; the catalog is now carried alongside the id everywhere it
//!   is stored or crosses the FFI, so the shift has nothing left to prevent.
...
use super::super::anilist::types::{FuzzyDate, MediaCoverImage, MediaItem, MediaTitle};
...
mod tests {
    ...
    #[test]
    fn the_id_is_carried_through_unshifted() {
        let movie: TmdbMovie = serde_json::from_str(MOVIE_JSON).unwrap();
        let series: TmdbSeries = serde_json::from_str(SERIES_JSON).unwrap();

        // AniList 550 also exists and is not Fight Club. What keeps the two
        // apart is the `Catalog` stored beside the id, not the id's value.
        assert_eq!(movie.into_media_item().unwrap().id, 550);
        assert_eq!(series.into_media_item().unwrap().id, 1396);
    }
