...
# Force CI=true to bypass macOS Finder AppleScript permission issues during DMG bundling
export CI=true
...
if [[ ! -f "$DMG_PATH" ]]; then
    FALLBACK_PATH="$PROJECT_ROOT/web/src-tauri/target/release/bundle/dmg/${DMG_NAME}"
...
if [[ "$MODE" == "--dry-run" ]]; then
    echo "Dry run complete. DMG built at: $DMG_PATH"
    echo "Use --nightly or --stable to upload."
    exit 0
fi

# Check if gh CLI is authenticated
if ! gh auth status &>/dev/null; then
    echo "WARNING: GitHub CLI (gh) is not authenticated. Skipping release upload."
    echo "You can authenticate by running: gh auth login"
    echo "The DMG has been built successfully and is located at: $DMG_PATH"
    exit 0
fi
