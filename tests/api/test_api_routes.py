...
def client_and_ctx(monkeypatch):
    ...
    monkeypatch.setattr("anicat_media.core.config.ConfigLoader", FakeLoader)
...
def test_test_provider_endpoint(client_and_ctx, monkeypatch):
    client, _ = client_and_ctx

    class FakeSearchResults:
        def __init__(self):
            self.results = [
                SimpleNamespace(id="naruto-1", title="Naruto Shippuden"),
                SimpleNamespace(id="naruto-2", title="Naruto (Original)"),
            ]
