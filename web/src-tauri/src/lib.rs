...
fn restart_backend(state: tauri::State<'_, SidecarState>) -> Result<String, String> {
    ...
    if let Some(ref mut child) = *child_guard {
        ...
        let _ = child.wait();
        log::info!("[sidecar] Killed backend process group for restart");
    }
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            let state_wd = app.state::<SidecarState>();
            let child_arc = state_wd.child.clone();
            let restart_count_arc = state_wd.restart_count.clone();
            let exiting_wd = state_wd.exiting.clone();
            ...
            let setup_result = (|| -> Result<(), Box<dyn std::error::Error>> {
                ...
                #[cfg(target_os = "macos")]
                {
                    ...
                    let _ = create_command("sh")
                        .arg("-c")
                        .arg("lsof -ti :13370 | xargs kill -9 2>/dev/null; pkill -9 -f anicat-server 2>/dev/null; pkill -9 -f 'input-ipc-server' 2>/dev/null; true")
                        .output();
                ...
                #[cfg(target_os = "macos")]
                {
                    ...
                    let window_menu = SubmenuBuilder::new(app, "Window")
                        .item(&MenuItemBuilder::with_id("show_main", "Show Dashboard")
                            .accelerator("CmdOrCtrl+Shift+D")
                            .build(app)?)
                        .item(&MenuItemBuilder::with_id("toggle_quick_pane", "Toggle Quick Pane")
                    ...
                    app.on_menu_event(move |_app, event| {
                        match event.id().as_ref() {
                            ...
                            "toggle_quick_pane" => {
                                if let Some(window) = app_handle.get_webview_window("quick-pane") {
                ...
                // Quick Pane can still be toggled via the Window menu.
                ...
                thread::spawn(move || loop {
                    ...
                    if exiting_wd.load(std::sync::atomic::Ordering::Relaxed) {
                        break;
                    }
                    ...
                    if let Some(state) = app.try_state::<SidecarState>() {
                        ...
                        if let Ok(mut guard) = state.child.lock() {
                            if let Some(ref mut child) = *guard {
                                ...
                                let _ = child.wait();
                                log::info!("[sidecar] Backend process group terminated");
                            }
                        }
                    }
                }
