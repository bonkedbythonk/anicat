...
pub async fn admin_status(
    State(state): State<ProxyState>,
    Query(q): Query<AdminStatusQuery>,
) -> Result<Json<Value>, StatusCode> {
    let configured = {
        let cfg = state.app_state.config.read().await;
        cfg.mobile.admin_key.clone()
    };
    match configured {
        Some(k) if !k.is_empty() && q.key.as_deref() == Some(k.as_str()) => {}
    ...
    let playback_map = state.app_state.inner.user_playback.lock().await;

    let mut entries: Vec<Value> = Vec::new();
    for (user_id, last) in seen.iter() {
        let secs_ago = last.at.elapsed().as_secs();
        let online = secs_ago < 90;
        // current_playback isn't cleared when a viewer just closes the app, so
        // it can hold a stale show long after they left. Only surface it while
        // the user is actually online, so `watching` never contradicts a
        // false `online`.
        let watching = match playback_map.get(user_id) {
            Some(pb_state) if online => {
                let guard = pb_state.current_playback.lock().await;
                guard.as_ref().map(|pb| serde_json::json!({
                    "media_id": pb.media_id,
                    "title": pb.title,
                    "episode_number": pb.episode_number,
                    "episode_title": pb.episode_title,
                    "total_episodes": pb.total_episodes,
                    "provider": pb.provider,
                    "position": pb.last_position,
                    "duration": pb.last_duration,
                    "paused": pb.paused,
                }))
            }
            _ => None,
        ...
        entries.push(serde_json::json!({
            "user_id": user_id,
            "name": names.get(user_id).cloned().unwrap_or_else(|| format!("user {user_id}")),
            "online": online,
    ...
    entries.sort_by_key(|e| e.get("seconds_ago").and_then(|v| v.as_u64()).unwrap_or(u64::MAX));

    Ok(Json(serde_json::json!({ "users": entries })))
}
