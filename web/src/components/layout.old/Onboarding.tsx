...
export default function Onboarding({ onComplete, onSkip }: OnboardingProps) {
    ...
    mediaApi.getConfig()
      .then((cfg) => {
        ...
        if (cfg?.stream) {
          ...
          setAutoplayTrailers(cfg.stream.autoplay_trailers !== false);
        }
      })
      .catch(console.error);
  ...
  const handleSavePlaybackSettings = () => {
    setError(null);
    const finalAutoSkip = playerType === "external" ? false : localAutoSkip;
    ...
    localStorage.setItem("anicat_auto_skip", String(finalAutoSkip));
          ...
          <div className="space-y-4 bg-white/[0.01] border border-white/[0.04] p-5 rounded-xl">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <option value="external" className="bg-surface" disabled={mpvAvailable === false}>
                  External Player - MPV (Premium Quality){mpvAvailable === false ? ' [Not Installed]' : ''}
                </option>
              </select>
              {mpvAvailable === false && (
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <option value="balanced" className="bg-surface">High Quality (Auto-Upscale to HD/4K)</option>
              </select>
            </div>
            ...
            <div className="space-y-2">
              ...
              <select
                value={playerType === "external" ? "false" : (localAutoSkip ? "true" : "false")}
                disabled={playerType === "external"}
            ...
            {/* Auto-Play Trailers */}
            <div className="space-y-2">
              <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">Autoplay Trailers</label>
              <select
                value={autoplayTrailers ? "true" : "false"}
                onChange={(e) => setAutoplayTrailers(e.target.value === "true")}
                className="w-full bg-white/[0.03] border border-white/[0.08] rounded-xl p-3.5 text-sm font-medium focus:border-accent/40 outline-none transition-all appearance-none cursor-pointer text-white"
              >
                <option value="true" className="bg-surface">Enabled (Play trailers in background)</option>
                <option value="false" className="bg-surface">Disabled</option>
              </select>
              <p className="text-[10px] text-gray-500 mt-1">
                Automatically plays muted preview videos for anime on the home screen and detail pages.
              </p>
            </div>
          </div>
