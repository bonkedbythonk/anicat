          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            {item.format && (
              <div>
                ...
                <p className="text-[var(--text-primary)]">{item.format}</p>
              </div>
            )}
            {item.status && (
              <div>
                <span className="text-[var(--text-muted)]">Status</span>
                <p className="text-[var(--text-primary)]">{item.status}</p>
              </div>
            )}
            {item.episodes && (
              <div>
                <span className="text-[var(--text-muted)]">Episodes</span>
                <p className="text-[var(--text-primary)]">{item.episodes}</p>
              </div>
            )}
            {item.averageScore && (
              <div>
                <span className="text-[var(--text-muted)]">Score</span>
                <p className="text-[var(--text-primary)]">{item.averageScore}%</p>
              </div>
            )}
          </div>
        </div>
