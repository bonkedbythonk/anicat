...
export default function AnimePlayer({
  ...
  const { playingProvider, playingServer } = useAppState();
  ...
  useEffect(() => {
    ...
    mediaApi.resolveStream(mediaId, episodeNumber, playingProvider || undefined, playingServer || undefined)
      .then(data => {
  ...
  }, [mediaId, episodeNumber, playingProvider, playingServer]);
    ...
    import("hls.js").then((M) => {
      ...
      if (canPlayNative) {
        ...
        const attemptStreamRenewal = async () => {
          ...
          try {
            const renewed = await mediaApi.renewStream(mediaId, episodeNumber, playingProvider || undefined);
            ...
            switch (data.type) {
              case Hls.ErrorTypes.NETWORK_ERROR:
                ...
                if (networkRecoverAttempts <= 3) {
                  ...
                  attemptStreamRenewal().catch((err: unknown) => {
                    console.error("[Player][C4] Stream renewal failed:", err);
  ...
  const handleOpenExternalMpv = () => {
    mediaApi.play(mediaId, episodeNumber, playingProvider || undefined)
      .then(() => onClose())
      .catch(err => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute top-0 inset-x-0 p-5 bg-gradient-to-b from-black/85 via-black/40 to-transparent flex items-center justify-between transition-all duration-500 z-30 ${
      ...
      <div
        ...
        className={`absolute bottom-0 inset-x-0 p-6 bg-gradient-to-t from-black/95 via-black/60 to-transparent flex flex-col space-y-4 transition-all duration-500 z-30 ${
          showControls ? "translate-y-0 opacity-100" : "translate-y-full opacity-0 pointer-events-none"
        }`}
      >
