  ...
  return (
    <div className="w-full max-w-lg mx-auto">
      <button
        ...
        className="inline-flex items-center space-x-2 px-4 py-2 rounded-xl bg-card hover:bg-surface border border-border text-xs font-bold text-gray-400 hover:text-foreground transition-all"
      >
        <Terminal size={12} />
      ...
      {showLogs && (
        <div className="mt-3 p-3 rounded-xl bg-background/80 border border-border max-h-48 overflow-y-auto text-left font-mono text-[10px] leading-relaxed whitespace-pre-wrap break-all text-gray-400 scrollbar-hide">
          {displayMessage()}
          <div ref={logEndRef} />
        </div>
      )}
