...
export function AniCatPlayer(props: AniCatPlayerProps) {
  ...
  const mediaDetailRef = useRef(mediaDetail);
  mediaDetailRef.current = mediaDetail;

  // Fallback for duration if resolve_builtin_player_stream didn't carry one,
  // without triggering a stream re-resolution.
  useEffect(() => {
    if (!expectedDurationRef.current && mediaDetail?.duration && mediaDetail.duration > 0) {
      const d = mediaDetail.duration * 60;
      expectedDurationRef.current = d;
      ...
      try {
        ...
        const resolvedDuration =
          typeof data.duration_seconds === "number" && data.duration_seconds > 0
            ...
            : (mediaDetailRef.current?.duration && mediaDetailRef.current.duration > 0
                ? mediaDetailRef.current.duration * 60
                : null);
        expectedDurationRef.current = resolvedDuration;
        if (resolvedDuration) {
  ...
  const lastResolvedEpisodeRef = useRef<number | null>(null);
  useEffect(() => {
    if (lastResolvedEpisodeRef.current === episodeNumber) return;
    lastResolvedEpisodeRef.current = episodeNumber;
  ...
  // Subtitle track synchronization (works for HLS text tracks, native Safari HLS, and <track> elements)
  useEffect(() => {
    const video = videoRef.current;
    if (!video) return;
    const sync = () => {
  ...
  }, [subtitlesEnabled, subtitleUrl, streamUrl]);

  // Sync Hls.js subtitle track state when toggling
  useEffect(() => {
    if (hlsRef.current && hlsRef.current.subtitleTracks && hlsRef.current.subtitleTracks.length > 0) {
      hlsRef.current.subtitleTrack = subtitlesEnabled ? 0 : -1;
    }
  }, [subtitlesEnabled]);
  ...
  const handleTimeUpdate = () => {
    ...
    // For a remuxed stream still being written, `dur` (video.duration)
    // is only the length of the HLS segments generated so far (an EVENT playlist).
    // The release's real total duration comes from the container header
    // (`expectedDurationRef.current`). We present `effectiveDur` to the user and seekbar
    // so the player shows the full episode duration from the start, matching external MPV,
    // while using `dur` to track what has been remuxed so far.
    const effectiveDur = Math.max(expectedDurationRef.current || 0, dur);
    ...
    // Buffer calculation: show how much is buffered / remuxed out of the total duration
    const bufferedEnd = video.buffered.length > 0 ? video.buffered.end(video.buffered.length - 1) : 0;
    const available = Math.max(bufferedEnd, dur);
    if (effectiveDur > 0) {
      setBuffered(Math.min(100, (available / effectiveDur) * 100));
    }
  ...
  return (
    <div
      ...
      className={`fixed inset-0 z-[9999] bg-background select-none overflow-hidden flex items-center justify-center font-sans ${
    ...
    >
      ...
      <video
        ...
        onEnded={() => {
          const effectiveDur = durationRef.current || 0;
          const cur = videoRef.current?.currentTime || 0;
          if (effectiveDur <= 0 || effectiveDur - cur <= 30) {
            handleNextEpisode();
          }
        }}
        onError={() => {
      ...
      {seekFeedback && (
        ...
        >
          <div className="flex flex-col items-center justify-center p-5 rounded-full bg-surface/90 text-foreground border border-border/50 backdrop-blur-md shadow-2xl">
      ...
      {playPauseFeedback && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-30 animate-scale-in">
          <div className="p-6 rounded-full bg-surface/90 text-foreground border border-border/50 backdrop-blur-md shadow-2xl">
      ...
      {(loading || buffering) && !error && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/60 backdrop-blur-xs z-20 space-y-4 pointer-events-none">
      ...
      {/* Error Overlay */}
      {error && !loading && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/90 backdrop-blur-md z-40 p-6 space-y-4 text-center">
          <AlertCircle className="w-14 h-14 text-danger" />
          <div className="max-w-md space-y-1">
            <p className="font-bold text-foreground text-lg">Playback Error</p>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="px-4 py-2 bg-foreground/10 hover:bg-foreground/20 text-foreground font-bold text-xs rounded-xl transition-all cursor-pointer flex items-center gap-1.5 border border-foreground/10"
            >
            ...
            <button
              onClick={handleClose}
              className="px-4 py-2 bg-foreground/10 hover:bg-foreground/20 text-foreground font-bold text-xs rounded-xl transition-all cursor-pointer border border-foreground/10"
            >
              Close
            </button>
          </div>
      ...
      {activeSkip && (
        <div className="absolute bottom-24 right-8 z-40 animate-slide-up">
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-accent text-background font-bold text-sm shadow-2xl hover:scale-105 active:scale-95 transition-all cursor-pointer border border-foreground/20"
          >
            <SkipForward size={16} />
      ...
      {outroCountdown !== null && !dismissOutro && (
        <div className="absolute bottom-24 left-8 z-40 animate-slide-up">
          <div className="flex items-center gap-3 p-3 rounded-2xl bg-surface border border-border backdrop-blur-md shadow-2xl">
            ...
            <button
              ...
              className="p-1.5 rounded-xl text-muted-foreground hover:text-foreground hover:bg-foreground/10 transition-colors cursor-pointer"
            >
              <X size={14} />
            </button>
          </div>
      ...
      {/* Top Header Chrome */}
      <div
        className={`absolute top-0 inset-x-0 z-30 p-4 sm:p-6 bg-gradient-to-b from-background/90 via-background/40 to-transparent flex items-center justify-between gap-4 transition-all duration-300 ${
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          <button
            onClick={handleClose}
            className="p-2 rounded-xl bg-foreground/10 hover:bg-foreground/20 text-foreground backdrop-blur-md transition-all cursor-pointer"
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleOpenInExternalMpv}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-foreground/10 hover:bg-foreground/20 text-foreground backdrop-blur-md border border-foreground/10 text-xs font-semibold transition-all cursor-pointer"
      ...
      <div
        className={`absolute bottom-0 inset-x-0 z-30 p-4 sm:p-6 bg-gradient-to-t from-background/95 via-background/60 to-transparent space-y-3 transition-all duration-300 ${
          chromeVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 pointer-events-none"
        }`}
      >
        {/* Timeline Scrubber */}
        <div
          ...
          onClick={(e) => {
            ...
            // Determine maximum seekable position
            ...
            const maxSeek = Math.max(seekableEnd, videoRef.current.duration || 0);
            const clampedTarget = Math.max(0, Math.min(target, maxSeek > 0 ? maxSeek : target));
            videoRef.current.currentTime = clampedTarget;
            setCurrentTime(clampedTarget);
          }}
          className="relative py-2.5 -my-2.5 cursor-pointer group flex items-center"
        >
          {/* Track Bar */}
          <div className="relative w-full h-2 group-hover:h-3.5 bg-foreground/15 rounded-full transition-all flex items-center">
            {/* Hover Time Tooltip */}
            {hoverTime !== null && hoverX !== null && (
              <div
                className="absolute -top-7 -translate-x-1/2 bg-surface text-foreground font-mono text-[11px] font-bold px-2 py-0.5 rounded-md border border-border pointer-events-none shadow-lg"
            ...
            {/* Buffered Progress */}
            <div
              className="absolute left-0 top-0 bottom-0 bg-foreground/25 rounded-full pointer-events-none transition-all"
            ...
            {/* Scrubber Thumb Dot */}
            <div
              className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-3.5 h-3.5 rounded-full bg-foreground shadow-md border-2 border-accent transition-transform pointer-events-none group-hover:scale-125"
              style={{
        ...
        <div className="flex items-center justify-between flex-wrap gap-2 text-foreground">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={togglePlay}
              className="p-2 rounded-xl hover:bg-foreground/15 text-foreground transition-colors cursor-pointer"
            ...
            <button
              ...
              className="p-1.5 rounded-lg hover:bg-foreground/15 disabled:opacity-30 text-foreground transition-colors cursor-pointer"
            ...
            <button
              onClick={handleNextEpisode}
              className="p-1.5 rounded-lg hover:bg-foreground/15 text-foreground transition-colors cursor-pointer"
            ...
            <div className="flex items-center gap-1.5 group/vol">
              ...
              <input
                ...
                className="w-16 sm:w-20 accent-accent h-1.5 rounded-lg bg-foreground/20 cursor-pointer"
              />
            </div>
            ...
            <button
              ...
              className="font-mono text-xs text-foreground/80 hover:text-foreground cursor-pointer select-none"
          ...
          {/* Right: Subtitles, Speed, Fullscreen */}
          <div className="flex items-center gap-2">
            {/* Subtitle Toggle */}
            {hasSubtitles && (
              <button
                onClick={() => setSubtitlesEnabled((s) => !s)}
                className={`p-1.5 rounded-lg hover:bg-foreground/15 transition-colors cursor-pointer ${
                  subtitlesEnabled ? "text-accent" : "text-foreground/40 hover:text-foreground"
                }`}
            ...
            <select
              ...
              className="bg-foreground/10 hover:bg-foreground/20 border border-foreground/10 rounded-lg px-2 py-1 text-xs font-mono font-bold text-foreground outline-none cursor-pointer appearance-none"
            >
              {SPEEDS.map((s) => (
      ...
      {showDrawer && (
        <div
          className="fixed inset-0 z-50 bg-background/60 backdrop-blur-xs flex justify-end animate-fade-in"
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2 text-xs font-mono">
              ...
              <div className="flex justify-between p-2 rounded-lg bg-foreground/[0.03]">
                ...
                <kbd className="px-1.5 py-0.5 bg-foreground/10 rounded">C</kbd>
              </div>
              <div className="flex justify-between p-2 rounded-lg bg-foreground/[0.03]">
