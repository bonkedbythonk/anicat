...
function SettingsView({ health, onUpdateStarted }: SettingsViewProps) {
  ...
  const handleBranchChange = async (newBranch: string) => {
    ...
    try {
      ...
      toast("Failed to update update branch");
    }
  };
  ...
  useEffect(() => {
    if (activeTab === "maintenance" && !registryStats) {
      mediaApi.getRegistryStats().then(setRegistryStats).catch(() => toast("Failed to load registry stats"));
    }
          ...
          {activeTab === "general" && (
            <div className="space-y-6 animate-fade-in">
                ...
                >
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    >
                      ...
                      <div className="px-3 py-2" style={{ background: "rgba(10, 132, 255, 0.04)" }}>
                        <div className="text-xs font-bold theme-title-neon-abyss" style={{ color: "#0A84FF" }}>Neon Abyss</div>
                    ...
                    >
                      ...
                      <div className="px-3 py-2" style={{ background: "rgba(244,180,196,0.04)" }}>
                        <div className="text-xs font-bold theme-title-sakura-zen" style={{ color: "#f2bfce" }}>Sakura Zen</div>
                        <div className="text-[10px] mt-0.5 theme-desc-sakura-zen" style={{ color: "#9ab89a" }}>Soft pastel / Japanese editorial</div>
                      </div>
                      {uiStyle === "sakura-zen" && (
                    ...
                    >
                      ...
                      <div className="px-3 py-2" style={{ background: "rgba(232, 39, 44, 0.04)" }}>
                        <div className="text-xs font-bold theme-title-retro-manga" style={{ color: "#e8272c" }}>Retro Manga</div>
                        <div className="text-[10px] mt-0.5 text-gray-500 theme-desc-retro-manga">Halftone dot / Manga panel style</div>
                      </div>
                      {uiStyle === "retro-manga" && (
                    ...
                    >
                      ...
                      <div className="px-3 py-2" style={{ background: "rgba(16, 185, 129, 0.04)" }}>
                        <div className="text-xs font-bold theme-title-forest-moss" style={{ color: "#10b981" }}>Forest Moss</div>
                        <div className="text-[10px] mt-0.5 text-gray-500 theme-desc-forest-moss">Organic green / Fresh woodland</div>
                      </div>
                      {uiStyle === "forest-moss" && (
              ...
              <CardSection title="Content Sources">
                ...
                >
                  <select
                    value={String(config.general?.provider || "gogoanime")}
                ...
                >
                  <div className="space-y-2">
                    {(() => {
                      ...
                      const allProviders = ["gogoanime", "anizone"];
                      ...
                      return slots.map((i) => {
                        ...
                        // Filter out the primary provider and already-selected fallbacks
                         const primary = String(config.general?.provider || "gogoanime");
            ...
            <div className="space-y-6 animate-fade-in">
              <CardSection title="Playback">
                <SettingField label="Skip Intro" description="Automatically skip opening and ending credits using AniSkip times.">
                  <select
                    value={config.stream?.player_type === "external" ? "false" : (autoSkip ? "true" : "false")}
                    disabled={config.stream?.player_type === "external"}
                    onChange={(e) => {
                      const val = e.target.value === "true";
                      setAutoSkip(val);
                      localStorage.setItem("anicat_auto_skip", String(val));
                    }}
                    className="w-full bg-white/[0.03] border border-white/[0.08] rounded-xl p-3.5 text-sm font-medium focus:border-accent/40 outline-none transition-all appearance-none cursor-pointer text-white disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                    <option value="false">Manual (Show Skip Intro button)</option>
                    <option value="true">Automatic (skip without prompt)</option>
                  </select>
                  {config.stream?.player_type === "external" && (
                    <p className="text-[10px] text-amber-400/80 font-medium mt-1">
                      Only available in the built-in player.
                    </p>
                  )}
                </SettingField>
              </CardSection>
              ...
              <CardSection title="Video Player">
                ...
                <SettingField label="1080p/4K GPU Upscaling (MPV only)" description="Anime4K real-time upscaling to 1080p/4K for the external player.">
                  <select
                    value={String(config.stream?.shader_profile || "balanced")}
                    onChange={(e) => updateField("stream", "shader_profile", e.target.value)}
                    className="w-full bg-white/[0.03] border border-white/[0.08] rounded-xl p-3.5 text-sm font-medium focus:border-accent/40 outline-none transition-all appearance-none cursor-pointer text-white"
                  >
                    <option value="balanced">Enabled (Anime4K)</option>
                    <option value="off">Disabled (Standard)</option>
                  </select>
                </SettingField>

                <SettingField label="Auto-Play Trailers" description="Play muted background trailer videos on featured and detail pages.">
                  <select
