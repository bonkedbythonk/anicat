...
export interface AppState {
  ...
  /** The provider chosen for the current playback session. */
  playingProvider: string | null;
  /** The server chosen for the current playback session. */
...
export interface AppStateActions {
  ...
  closeDetail: () => void;
  startPlayback: (item: MediaItem, episode: string, provider?: string, server?: string) => void;
  closePlayback: () => void;
...
export function AppStateProvider({ children }: { children: ReactNode }) {
  ...
  const startPlayback = useCallback((item: MediaItem, episode: string, provider?: string, server?: string) => {
    setPlayingItem(item);
    setPlayingEpisode(episode);
    setPlayingProvider(provider || null);
    setPlayingServer(server || null);
  }, []);
