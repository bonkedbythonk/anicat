...
export const mediaApi = {
  ...
  getEpisodes: (mediaId: number, provider?: string): Promise<Episode[]> =>
    fetchFromApi(`/media/${mediaId}/episodes${provider ? `?provider=${provider}` : ""}`),
  ...
  // ─── Playback ───────────────────────────────────────
  play: async (mediaId: number, episode?: string, provider?: string, server?: string) => {
    const params = new URLSearchParams();
  ...
  resolveStream: (mediaId: number, episode?: string, provider?: string, server?: string): Promise<{
    stream_url: string;
    ...
    if (server) params.append('server', server);
    const queryString = params.toString() ? `?${params.toString()}` : '';
    return fetchFromApi(`/actions/play/${mediaId}/resolve${queryString}`, { method: 'POST' });
  },

  getStreams: (mediaId: number, episode: string, provider?: string): Promise<{
    streams: Array<{
      server: string;
      episode: string;
      links: Array<{
    ...
    if (provider) params.append('provider', provider);
    return fetchFromApi(`/actions/play/${mediaId}/streams?${params.toString()}`);
  },

  // C4: Re-resolve an expired/failed stream URL without full playback re-init
  renewStream: (mediaId: number, episode: string, provider?: string): Promise<{
    stream_url: string;
    raw_stream_url: string;
    title: string;
    episode: string;
    ...
    fetchFromApi('/actions/stream/renew', {
      method: 'POST',
      body: JSON.stringify({ media_id: mediaId, episode, provider })
    }),
  ...
  playNext: async (mediaId: number, provider?: string) => {
    ...
    if (provider) params.append('provider', provider);
    return fetchFromApi(`/actions/play/${mediaId}?${params.toString()}`, { method: 'POST' });
  },
  ...
  testProvider: (providerName: string, isManga: boolean, query: string): Promise<{
    ...
    fetchFromApi('/status/test-provider', {
      method: 'POST',
      body: JSON.stringify({ provider_name: providerName, is_manga: isManga, query })
    }),
};
